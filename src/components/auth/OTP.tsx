...
const OtpPage = () => {
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      // You no longer need to retrieve otpToken from sessionStorage
      const response = await axios.post(
        "https://viabtesting.onrender.com/api/v1/users/verify-signup-user-otp",
        { otp: fullOtp },
        {
  ...
  const handleResendOtp = async () => {
    ...
    if (!email) {
      alert("No email found in session. Please sign up again.");
      return;
    }
    ...
    try {
      setResending(true);
      const response = await axios.post(
        "https://viabtesting.onrender.com/api/v1/users/resend-otp",
         { email },
        { withCredentials: true }
      );
      ...
      const err = error as AxiosError<{ message: string }>;
      if (err.response) {
        alert(`Error: ${err.response.data?.message || "Failed to resend OTP."}`);
      } else {
        alert("Network/server error: " + err.message);
      }
    } finally {
  ...
  //     const response = await axios.post(
  //       `${baseURL}/users/verify-signup-user-otp`,
  //       { otp: fullOtp },
  //       {
  //         headers: {
  //           "x-auth-otp": otpToken,
      ...
      <div className="otp-card shadow p-4 rounded p-5">
        ...
        <form onSubmit={handleSubmit} className="d-flex flex-column align-items-center">
          ...
          <p className="text-center small">
            Didn&apos;t receive code?{" "}
            <span
              className={`resend-link ${resending || resendCooldown > 0 ? "disabled" : ""}`}
              style={{ cursor: resending || resendCooldown > 0 ? "not-allowed" : "pointer" }}
              onClick={() => {
                if (!resending && resendCooldown === 0) handleResendOtp();
              }}
            >
              {resending
                ? "Sending..."
