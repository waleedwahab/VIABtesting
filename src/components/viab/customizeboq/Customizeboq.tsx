...
const Customizeboq = () => {
  ...
  return (
    <div className="container-fluid insuranceSection py-4 pb-5">
      <div className="container py-4 px-3 px-md-4 second-layer">
        <div className="boq-title">Customize your BoQ</div>

        <label className="boq-label">Usage Type</label>
        <input
          className="form-control boq-input  mb-3"
          value={usageType}
        ...
        <div className="boq-btn-group d-flex flex-wrap">
          {materials.map((item, i) => (
            <button
              key={i}
              className="btn btn-outline-secondary pe-4 pe-md-5 new-color pt-3 pb-3"
            >
              <div>
                <span className="me-1">
                  <Image src={item.image} width={30} height={30} alt="Material" />
                </span>
                {item.title}
              </div>
            </button>
        ...
        <div className="w-100 text-end">
          <button className="btn btn-primary generate-btn">Generate BoQ</button>
        </div>
      </div>
    </div>
  );
